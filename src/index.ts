...
import { deleteFile, list, status, upload } from './commands.js'
...
async function main() {
    ...
    if (command === 'status') {
        ...
        const batchId = Binary.hexToUint8Array(Types.asHexString(env.SWARMFS_BATCH_ID))
        for (const { path, rootHash, kind } of list({ batchId, stateDir })) {
            console.log(`${Binary.uint8ArrayToHex(rootHash)}  ${path}  [${kind}]`)
        }
        ...
        const batchDepth = Types.asNumber(env.SWARMFS_BATCH_DEPTH)
        const path = resolve(Types.asString(argv[3]))
        let lastFile = ''
